...
def read_input():
    ...
    with sideb:
        ...
        with c1:
            pres = st.text_input("Initial pressure (bar):", 350.0)
            pres = float(pres) * 1e5

            back_pressure = st.text_input("Fill/back pres. (bar):", 1)
            ...
            mode = st.selectbox("Select mode", ("discharge", "filling"))
            temp = st.text_input("Initial temp. (C):", 25)
            temp = float(temp) + 273.15
            end_time = st.text_input("End time (s):", 240)

        Cd = st.text_input("Orifice discharge coefficient:", 0.84)
    ...
    input["valve"]["discharge_coef"] = float(Cd)
...
if __name__ == "__main__":
    ...
    if st.sidebar.button("Run Simulation", type="primary"):
        with st.spinner("Calculating, please wait...."):
            ...
            ax.plot(
                temp_data["Time (s)"],
                temp_data["Fluid temperature (C)"],
                "k",
                label="Fluid",
            )
            ax.plot(
                temp_data["Time (s)"],
                temp_data["Wall temperature (C)"],
                "k--",
                label="Wall",
            )
            if input["valve"]["flow"] == "discharge":
        ...
        **Disclaimer**
        This app and the underlying code is provided free of charge under an [MIT license](https://github.com/ORS-Consulting/HydDown/blob/main/LICENSE). 
